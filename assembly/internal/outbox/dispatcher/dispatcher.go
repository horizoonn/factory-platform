...
func (d *Dispatcher) handlePublishError(
	...
	if event.Attempts >= d.config.MaxAttempts {
		if err := d.repository.MarkFailed(ctx, d.config.WorkerID, event.ID, lastError); err != nil {
			return fmt.Errorf("mark outbox event failed: %w", err)
		}
		logger.Error(
			ctx, "outbox event exhausted retries",
			zap.String("event_id", event.ID.String()),
			zap.String("topic", event.Topic),
			zap.Int("attempt", event.Attempts),
	...
	logger.Warn(
		ctx, "outbox event publish failed; retry scheduled",
		zap.String("event_id", event.ID.String()),
		zap.String("topic", event.Topic),
		zap.Int("attempt", event.Attempts),
