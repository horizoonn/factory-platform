...
func TestOutboxRepository_Enqueue(t *testing.T) {
	...
	created, err := repository.Enqueue(testContext(t), testEnv.pool, event)
	...
	err = testEnv.pool.QueryRow(testContext(t), `
		SELECT id, aggregate_id, event_type, topic, message_key,
			payload, headers, available_at, next_attempt_at
		FROM platform.order_outbox_events
		WHERE id = $1
	`, event.ID).Scan(
		&id,
		&aggregateID,
	...
	assert.Equal(t, event.Type, eventType)
...
func TestOutboxRepository_ClaimAndMarkPublished(t *testing.T) {
	...
	assert.Equal(t, event.AggregateID, claimed.AggregateID)
	assert.Equal(t, event.Type, claimed.Type)
	assert.Equal(t, event.Topic, claimed.Topic)
	assert.Equal(t, event.Key, claimed.Key)
	assert.Equal(t, event.Payload, claimed.Payload)
	assert.Equal(t, event.Headers, claimed.Headers)
...
func TestOutboxRepository_RescheduleAndMarkFailed(t *testing.T) {
	...
	_, err = testEnv.pool.Exec(
		testContext(t),
		"UPDATE platform.order_outbox_events SET next_attempt_at = NOW() - INTERVAL '1 second' WHERE id = $1",
		event.ID,
	)
	require.NoError(t, err)
	...
	err = testEnv.pool.QueryRow(
		testContext(t),
		"SELECT failed_at FROM platform.order_outbox_events WHERE id = $1",
		event.ID,
	).Scan(&failedAt)
	require.NoError(t, err)
...
func TestOutboxRepository_ExpiredLeaseCanBeReclaimed(t *testing.T) {
	...
	_, err = testEnv.pool.Exec(
		testContext(t),
		"UPDATE platform.order_outbox_events SET locked_until = NOW() - INTERVAL '1 second' WHERE id = $1",
		event.ID,
	)
	require.NoError(t, err)
...
func outboxFixture() outbox.Event {
	...
	return outbox.Event{
		...
		AggregateID: uuid.MustParse("7d4a1f4f-07cc-48b2-b7c7-f6201f983002"),
		Type:        "events.v1.OrderPaid",
		Topic:       "order.paid.v1",
		Key:         []byte("7d4a1f4f-07cc-48b2-b7c7-f6201f983002"),
